...
<div class="container" style="height:630px;overflow-x:auto;">
    ...
    <div class="span11" style=" border: solid; border-color: #DDDDDD;">
        ...
        <a href="#lvke" style="margin-left:20px;margin-top:10px;" data-toggle="modal" class="btn btn-info btn-small"
           onclick="souSuo()">
        ...
        <form id="form1" method="post" onsubmit="return verify()">
            ...
            <input id="lvKeId" name="id" type="hidden" value="0">                                       <!-- 旅客ID -->
            <input id="stayRegisterId" name="stayRegisterID" type="hidden" value="${stayRegisterId}">
            <!-- 住宿登记ID -->
            ...
            <div class="span12">
                <div class="row-fluid">
                    ...
                    <div class="span3">
                        <label>性别：</label>
                        <select id="genderId" name="gender.itemDetailsID" onchange="gender(this.value)"
                                style="width:100%;height:27px;">
                            <c:forEach items="${listGender}" var="item">
                                <option value="${item.itemDetailsID}"
                                        <c:if test="${item.itemDetailsID==33}">selected="selected"</c:if>>
                                        ${item.category}
                                </option>
                            </c:forEach>
                        </select>
                    ...
                    <div class="span3">
                        <label>民族：</label>
                        <select id="nationId" name="nation.itemDetailsID" onchange="nation(this.value)"
                                style="width:100%;height:27px;">
                            <c:forEach items="${listNation}" var="item">
                                <option value="${item.itemDetailsID}"
                                        <c:if test="${item.itemDetailsID==83}">selected="selected"</c:if>>
                                        ${item.category}
                                </option>
                            </c:forEach>
                        </select>
            ...
            <div class="span12">
                <div class="row-fluid">
                    <div class="span3">
                        <label>发证机关：</label>
                        <input id="faZhengId" name="licenceIssuingAuthorty"
                               onblur="licenceIssuingAuthortyOne(this.value)" type="text"
                               style="width:100%;height:27px;">
                    </div>
                    <div class="span3">
                        <label>证件有效期：</label>
                        <input id="papersValidityId" name="papersValidity" onblur="papersValidityOne(this.value)"
                               type="text" style="width:100%;height:27px;" placeholder="年-月-日 至 年-月-日">
                    </div>
                    <div class="span3">
                        <label>职业：</label>
                        <input id="professionId" name="profession" onblur="professionOne(this.value)" type="text"
                               style="width:100%;height:27px;">
                    </div>
                    <div class="span3">
                        <label>文化程度：</label>
                        <select id="educationDegreeId" name="educationDegree.itemDetailsID"
                                onchange="educationDegree(this.value)" style="width:100%;height:27px;">
                            <c:forEach items="${listEducationDegree}" var="item">
                                <option value="${item.itemDetailsID}"
                                        <c:if test="${item.itemDetailsID==51}">selected="selected"</c:if>>
                                        ${item.category}
                                </option>
                            </c:forEach>
                        </select>
            ...
            <div class="span12">
                <div class="row-fluid">
                    <div class="span3">
                        <label>旅客级别：</label>
                        <select id="passengerLevelId" name="passengerLevel.itemDetailsID"
                                onchange="passengerLevel(this.value)" style="width:100%;height:27px;">
                            <c:forEach items="${listPassengerLevel}" var="item">
                                <option value="${item.itemDetailsID}"
                                        <c:if test="${item.itemDetailsID==56}">selected="selected"</c:if>>
                                        ${item.category}
                                </option>
                            </c:forEach>
                        </select>
                    ...
                    <div class="span3">
                        <label>证件类型：</label>
                        <select id="papersId" name="papers.itemDetailsID" onchange="papers(this.value)"
                                style="width:100%;height:27px;">
                            <c:forEach items="${listPapers}" var="item">
                                <option value="${item.itemDetailsID}"
                                        <c:if test="${item.itemDetailsID==64}">selected="selected"</c:if>>
                                        ${item.category}
                                </option>
                            </c:forEach>
                        </select>
                    ...
                    <div class="span3">
                        <label>单位或住址：</label>
                        <input id="unitsOrAddressId" name="unitsOrAddress" onblur="unitsOrAddressOne(this.value)"
            ...
            <div class="span12">
                <div class="row-fluid">
                    <div class="span3">
                        <label>出生日期：</label>
                        <input id="birthDateId" name="birthDate" onblur="birthDateOne(this.value)" type="text"
                               style="width:100%;height:27px;" placeholder="年-月-日">
                    </div>
                    <div class="span3">
                        <label>事由：</label>
                        <select id="thingReasonId" name="thingReasonID.itemDetailsID" onchange="thingReason(this.value)"
                                style="width:100%;height:27px;">
                            <c:forEach items="${listThingReason}" var="item">
                                <option value="${item.itemDetailsID}"
                                        <c:if test="${item.itemDetailsID==67}">selected="selected"</c:if>>
                                        ${item.category}
                                </option>
                            </c:forEach>
                        </select>
                    ...
                    <div class="span3">
                        <label>从何处来：</label>
                        <input id="whereAreFromId" name="whereAreFrom" onblur="whereAreFromOne(this.value)" type="text"
                               style="width:100%;height:27px;">
                    </div>
                    <div class="span3">
                        <label>到哪里去：</label>
                        <input id="whereToGoId" name="whereToGo" onblur="whereToGoOne(this.value)" type="text"
                               style="width:100%;height:27px;">
                    </div>
                </div>
            </div>
            ...
            <div class="span12">
                <div class="row-fluid">
                    <div class="span3">
                        ...
                        <input id="contactPhoneNUmberId" name="contacPhoneNumber"
                               onblur="contactPhoneNUmberOne(this.value)" style="width:100%;height:27px;" type="text"
            ...
            <div class="span12">
                ...
                <input id="remarksId" name="remarks" onblur="remarksOne(this.value)" type="text"
                       style="width:100%;height:27px;">
            </div>
                ...
                <div class="row-fluid">
                    <div class="span8">
                        ...
                        <input id="txtnameid" name="txtname" class="textone"
                               style="width:55%; border-radius:0px; border-top-left-radius:4px; border-bottom-left-radius:4px;height:27px;"
                               type="text" placeholder="请输入关键字" value="${txtname}">
                        <div class="input-append">
                            <button onclick="souSuo()" class="btn-success textone"
        ...
        $.ajax({
            ...
            success: function (result) {
                for (var key in result) {
                    ...
                    tdGender.innerHTML = item.gender.category;
                    tdPapersName.innerHTML = item.papers.category;         //中间这个是数据
            ...
            if (chk_value.toString().indexOf(",") > 0) {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        document.getElementById("genderId").value = result.gender.itemDetailsID;
                        document.getElementById("nationId").value = result.nation.itemDetailsID;
                        ...
                        document.getElementById("educationDegreeId").value = result.educationDegree.itemDetailsID;
                        document.getElementById("passengerLevelId").value = result.passengerLevel.itemDetailsID;
                        document.getElementById("papersId").value = result.papers.itemDetailsID;
                        ...
                        document.getElementById("thingReasonId").value = result.thingReasonID.itemDetailsID;
                        ...
                        document.getElementById("contactPhoneNUmberId").value = result.contacPhoneNumber;
                        ...
                        xingBie = result.gender.itemDetailsID;
                        minZu = result.nation.itemDetailsID;
                        faZhengJiGuang = result.licenceIssuingAuthorty;
                        zhengJiaYouXiaoQi = result.papersValidity;
                        jiYe = result.profession;
                        wenHuaChengDu = result.educationDegree.itemDetailsID;
                        lvKeJiBie = result.passengerLevel.itemDetailsID;
                        zhengJianLeiXing = result.papers.itemDetailsID;
                        zhengJianHaoMa = result.papersNumber;
                        danWeiHuoZhuChu = result.unitsOrAddress;
                        chuShengRiQi = result.birthDate;
                        shiYou = result.thingReasonID.itemDetailsID;
                        congHeChuLai = result.whereAreFrom;
                        daoNaLiQu = result.whereToGo;
                        lianXiDianHua = result.contacPhoneNumber;
