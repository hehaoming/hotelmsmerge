        ...
        <form action="${ctx}/StayRegister/add.do" method="post" onsubmit="return verify()">
            ...
            <div class="span8">
                <div class="row-fluid">
                    ...
                    <input id="txtnameidTwo" name="txtname"
                           style="width:28%; border-radius:0px; border-top-left-radius:4px; border-bottom-left-radius:4px;height:27px;"
                           type="text" placeholder="请输入关键字">
                    <div class="input-append">
                        <button type="button" class="btn-success" style="margin-left:-4px;height:27px;"
            ...
            <div class="span12" style="height:262px;overflow-x:auto;">
                <table class="table table-condensed table-bordered table-striped" id="tableid">
                    ...
                    <tbody id="tbody">
                    <c:forEach items="${listRT}" var="teamItem">
                        <c:if test="${teamItem.targetType.itemDetailsID!=72}">
                            <tr>
                                <td><input type="radio" name="id" value="${teamItem.tuanduiID}"></td>
                                <td>${teamItem.teamName}</td>
                                <td>${teamItem.teamCode}</td>
                                <td>${teamItem.principal}</td>
                                <td>${teamItem.registerTime}</td>
                                <td>${teamItem.contactPhoneNUmber}</td>
                            </tr>
                        </c:if>
                    </c:forEach>
                    </tbody>
...
<script type="text/javascript">
    ...
    function selectTarget() {
        ...
        $.ajax({
            cache: false,
            type: "POST",
            url: '${ctx}/StayRegister/selectTarget.do',
            data: "txtname=" + name,
            async: false,
    ...
    function changOver() {
        ...
        if (chk_value != "") {
            parent.document.getElementById('Mainid').src = '${ctx}/StayRegister/changOver.do?id=' + id +
                '&receiveTargetID=' + chk_value + '&LvKeLeiXingId=' + 72;
        } else {
            alert("请选择一个团队！");
        }
    }
