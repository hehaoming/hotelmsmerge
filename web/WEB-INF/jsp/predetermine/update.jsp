...
<!--   <link rel="stylesheet" href="${ctx}/css/roomset/roomset.css" type="text/css"></link> -->
  ...
  <div class="container" style="height:630px;overflow-x:auto;border: solid; border-color: #DDDDDD;">
    ...
    <input id="teamId" type="hidden" value="${predetermine.type}" >  <!-- 是团队还是旅客 -->
      <input id="predetermineID" type="hidden" value="${predetermine.predetermineID}" >  <!-- 修改时作为参数传回，便于删除 -->
    ...
    <div class="span12" style="margin-top:12px;">
      <div class="row-fluid">
        <div class="span2">
          ...
          <c:if test="${predetermine.type==1}"> value="${predetermine.receptionName}" </c:if>
            <c:if test="${predetermine.type==2}"> value="${predetermine.personName}" </c:if> >
        </div>
        <div class="span2">
           <label>预订天数：</label>
        ...
        <div class="span2">
           ...
           <select name="payWayID" style="height:26px;width:100%;"> 
		            <c:forEach items="${listOne}" var="item">
			          <option value="${item.itemDetailsID}" <c:if test="${item.itemDetailsID==predetermine.payWayID}" >selected="selected"</c:if>>
			            ${item.category}
			          </option>
			        </c:forEach> 
		    </select>
         ...
         <div class="span3">
          ...
          <input id="arriveTimeId" name="arriveTime" style="height:26px;float:left;" id="date" class="Wdate" type="text" onchange="onchangeOne()"
		       onFocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{\'%y-%M-%d\'}',onpicked:pickedFunc})" value="${predetermine.arriveTime}"/>
